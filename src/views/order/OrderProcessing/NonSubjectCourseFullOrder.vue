...
import SelectCourse from "@/views/order/components/FullOrder/NonSubjectCourse/index";
...
export default {
  ...
  methods: {
      ...
      if (this.orderId) {
        findProductByOrder(this.orderId).then(res => {
          for (let i = 0; i < res.data.orderCourseList.length; i++) {
            if (
              res.data.orderCourseList[i].courseProperty == 5 &&
    ...
    getLocalStorageChoiceCourse() {
      ...
      if (courseArr.length > 0) {
        for (let i = 0; i < courseArr.length; i++) {
          if (courseArr[i][0].courseProperty === 5) {
    ...
    prevClick() {
      this.$router.push({
        path: prevClickPath(this.courses, 4, this.studentId),
