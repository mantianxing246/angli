...
-->
<template>
  <dialog-container title="选择直播课程" :visible.sync="dialog" width="60%">
    <template slot="body">
...
export default {
  ...
  data () {
    return {
      ...
      listCourse: [],
      choiceCourse: [],
      choiceCourseArr: Object.assign(this.liveSelectCourse)
    }
  },
  methods: {
    productCourse () {
  ...
  computed: {
    dialog: {
      get () {
        return this.liveSelectDialog
      },
      set (val) {
        this.$emit("update:liveSelectDialog", val)
        this.searchInfo = {
          classType: null,
          gradeId: null,
          version: null,
  ...
  props: ["liveSelectDialog", "headInfo", "liveSelectCourse"],
  watch: {
    liveSelectCourse (newVla) {
      this.choiceCourseArr = Object.assign(this.liveSelectCourse)
    }
  }
}
</script>
