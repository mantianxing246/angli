...
<template>
  <div>
    <dialog-container title="编辑产品组合" :visible.sync="dialogShow" width="70%">
      <template slot="body">
        <el-form ref="proForm" inline :rules="rules" :model="productQuery">
          ...
          <el-form-item label="产品名称" prop="productName">
            <el-input v-model="productQuery.productName" maxlength="50" clearable/>
          </el-form-item>
    ...
    flex-wrap: wrap;">
        ...
        <div>
          ...
          >
            ...
            <!-- <el-table-column label="高考年份" prop="year" width="100" align="center"/> -->
    ...
    <other-select-course
      ...
      :headInfo="headInfo"
    ></other-select-course>
    <!-- <consul-select-course
      :consulSelectDialog.sync="dialog.consulSelectDialog"
      @choiceCourse="offlineChoiceCourse"
    ></consul-select-course>-->
  </div>
</template>
<script>
import { findProductById, updateProduct } from "@/api/product/ProductMgtApi";
...
export default {
  name: "ProductAdd",
  props: ["productEditDialog", "findAllProduct"],
  components: {
    OfflineSelectCourse,
    RecSelectCourse,
    LiveSelectCourse,
  ...
  methods: {
    ...
    findProductById(id) {
      ...
      findProductById(id).then(res => {
        res.data.gradeList.forEach(element => {
          grade.push(element.gradeId);
        });
        this.productQuery = Object.assign({}, res.data, { grade: grade });
        this.choiceCourse = res.data.productCourses;
      });
    },
    removeCourse(index) {
  ...
  computed: {
    dialogShow: {
      set(val) {
        ...
        this.$emit("update:productEditDialog", val);
      },
      get() {
        return this.productEditDialog;
      }
    }
  },
  created() {
